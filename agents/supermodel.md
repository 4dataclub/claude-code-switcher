...
RESP=$(curl -sS --max-time 180 -X POST http://localhost:8091/api/generate \
  ...
  -d '{"category":"'"$KIND-$POOL"'","service":"claude-supermodel","prompt":"<FULL self-contained task>"}')
```

**PFLICHT-CHECK direkt danach — BEVOR du irgendetwas anderes tust:**
```bash
echo "$RESP" | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if 'text' in d else 1)"
```
- Exit 0 (`.text` vorhanden) → use `.text`, weiter.
- Exit 1 (`.error` oder kein `.text`) → **SOFORT returnen:** `DELEGATION FEHLER ($KIND-$POOL): cascade exhausted — kein Modell verfügbar. Bitte API-Keys prüfen oder Pool wechseln.` — **KEIN weiterer Tool-Call, KEINE Selbst-Implementierung, KEIN Fallback.**

Response JSON bei Erfolg: `{"text":"...","model":"...","latencyMs":...}` → use `.text`.
...
- **cloud / free fail-open:** if the `{kind}-cloud|free` call fails, report exactly `DELEGATION FEHLER (${kind}-${POOL}): cascade exhausted — kein Modell verfügbar. Bitte API-Keys prüfen oder Pool wechseln.` — Opus meldet das dem User und implementiert NICHT selbst. Der Orchestrator orchestriert, er implementiert nicht.
- **NEVER cross-pool:** if POOL=cloud, only call `{kind}-cloud`. NEVER fall back to `{kind}-local` or `{kind}-free`. Pool is fixed for the entire task. Cross-pool fallback is forbidden.
